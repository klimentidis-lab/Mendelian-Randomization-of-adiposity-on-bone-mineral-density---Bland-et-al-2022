setwd('/xdisk/yann/mig2020/rsgrps/yann/Victoria/GWAS_BodyFatPer_SexCombo/Males')
dat2 <- read.table('bolt_460K_selfRepWhite.PerBF_Male_Gwas.bgenv3.stats.gz', header=T)
...
#1372 2015 1705  317  643 1784  647  532 1172  571 1048  856  331  216 1259 1021 
#17   18   19   20   21   22   23 
#1031  422  455  670    1  305 1387 

save(dat2, file='FullGWASresults_PerBF_Male_GWAS.rda')
write.table(dat2, 'FullGWASresults_PerBF_Males.txt', quote=F, row.names=F, sep=' ')
...
write.table(top, 'TopHits_PerBF_Males_GWAS.txt', quote=F, row.names=F, sep='\t')
...
###################################################################################################
    ...
    write.table(dat3,file='LDHUB_PerBF_Male_GWAS_BOLTLMM_500K.txt', row.names=F, quote=F, sep='\t')
    
    #to zip: 
    system("zip LDHUB_PerBF_Male_GWAS_BOLTLMM_500K LDHUB_PerBF_Male_GWAS_BOLTLMM_500K.txt")
    ################################################################################################################################
    
    
    library(qqman)
    
    load('/xdisk/yann/mig2020/rsgrps/yann/Victoria/GWAS_BodyFatPer_SexCombo/Males/FullGWASresults_PerBF_Male_GWAS.rda')
    ...
    png(file='manhattan_PerBF_Males_9.png', res = 300, width = 10, height = 6,units = 'in')
    manhattan(myGWAS3, col = c("olivedrab4", "olivedrab3"), ylim=c(2,9))
    dev.off()
    #####
    ####
    png(file='manhattan_PerBF_Males_20.png', res = 300, width = 10, height = 6,units = 'in')
    manhattan(myGWAS3, col = c("olivedrab4", "olivedrab3"), ylim=c(5,20))
    dev.off()
    
    png(file='manhattan_PerBF_Males_50.png', res = 300, width = 10, height = 6,units = 'in')
    manhattan(myGWAS3, col = c("olivedrab4", "olivedrab3"), ylim=c(5,50))
    dev.off()
    
    png(file='QQ_PerBF_Males.png',res = 300, width = 10, height = 6,units = 'in')
...
###################################################################################################
    ...
    write.table(dat.fa,file='FAVariants_PerBF_Males_GWAS.txt', row.names=F, quote=F)
